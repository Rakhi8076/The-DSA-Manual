...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDigest = async () => {
      ...
      if (!userId) return;
      try {
        const res = await getDailyDigest(userId);
        if (res.digest) {
      ...
      <div className="flex-1 flex justify-center p-6">
        <div className="w-full max-w-5xl">
            ...
            <div className="bg-white/10 p-5 rounded-2xl">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center gap-4">
                  ...
                  <span className="text-sm font-medium truncate">{user?.email || "Not available"}</span>
                </div>
              </div>
            </div>
          ...
          <div className="mb-6 rounded-2xl p-6 bg-white/10 shadow-lg">
            ...
            {digest ? (
              <>
                <p className="text-sm text-gray-300 mb-4 leading-relaxed">{digest}</p>
                {stats && (
                  <div className="flex gap-6 text-sm text-gray-300">
                    <div className="flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-blue-400 inline-block"></span>
                      <span>{stats.problemsSolved} problems</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-gray-400">⏱</span>
                      <span>{stats.timeSpent} mins</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span>🔥</span>
